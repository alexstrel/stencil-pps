...
#pragma unroll
	...
	// Bwd neighbour contribution:
	{
      ...
      for (int s = 0; s < nSpin; s++){
        out(x,y,s) = (in(x,y,s) - tmp[s]);
      }      
#else      
    //
{
    std::array<int, nDir> X{x, y};    
    if(X[0] == (in.extent(0) - 1)) {
      X[0] = 0;    
      
      const Spinor in1_{in(X[0],X[1],0), in(X[0],X[1],1)};          
      const Spinor p1 = proj<+1>(in1_, 0);
#if 1      
      tmp[0] = (U(x,y,0) * p1[0]);	
      //
      tmp[1] = (U(x,y,0) * p1[1]);		
...
{   
    ...
    if( X[0] == 0 ) {
      ...
      const Spinor in2_{in(X[0],X[1],0), in(X[0],X[1],1)};          
      const Spinor p2 = proj<-1>(in2_, 0); 
      
      tmp[0] += (conj(U(X[0],X[1],0)) * p2[0]);	
      //
      tmp[1] += (conj(U(X[0],X[1],0)) * p2[1]);		
...
////////////////////////////////////////////////////////////////
{
    ...
    if(X[1] == (in.extent(1) - 1)) {
      X[1] = 0;    
      
      const Spinor in3_{in(X[0],X[1],0), in(X[0],X[1],1)};   
      const Spinor p3 = proj<+1>(in3_, 1);
...
{    
    ...
    if(X[1] == 0) {
      ...
      const Spinor in4_{in(X[0],X[1],0), in(X[0],X[1],1)};    
      const Spinor p4 = proj<-1>(in4_, 1);
      
      tmp[0] += (conj(U(X[0],X[1],1)) * p4[0]);	
      //
      tmp[1] += (conj(U(X[0],X[1],1)) * p4[1]);		
