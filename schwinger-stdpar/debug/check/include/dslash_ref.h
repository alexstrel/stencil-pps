...
template<typename Float>
void Dpsi_parity(auto &out_spinor, const auto &in_spinor, const auto &in_spinor_par, const auto &gauge_field, const Float mass, const int nxh, const int ny, const int parity) {
  ...
  const auto in    = in_spinor.Accessor();
  //
  const auto in_par= in_spinor_par.Accessor();  
  //
  const auto gauge = gauge_field.ExtAccessor(); 
