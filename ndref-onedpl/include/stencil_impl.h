...
(std::vector<T> &out, const std::vector<T, Allocator> &in, const int Lx, const int Ly, const int Lz, const float C0, const float C1)
{
...
template <ArithmeticTp data_tp, typename Allocator, int D, ArithmeticTp... coeffs>
class GenericNDStencilArg {
  ...
  public:
    ...
    GenericNDStencilArg(std::vector<T, Allocator> &out_, const std::vector<T, Allocator> &in_, const std::array<int, D> dims, const coeffs& ...c_) :
    	accessor_args(dims),
    	out(out_.data(), accessor_args),
	in (const_cast<std::vector<T, Allocator>&>(in_).data(), accessor_args),	
	c{c_...} { 
    }   
    //
...
class GenericNDStencil {
  ...
  inline Tp add_corner_neighbors(const int face_type, const std::array<int, D> &x, const int i) {
    ...
    //
    auto neigh  = is_curr_dir_bndry == 0 ? arg.in.template operator()<shifts[init_dir], shifts[base_dir], shifts[dir] > (i) :  arg.in.template get_bndry_term<dir>(x,i);
  ...
  inline Tp add_edge_neighbors(const int face_type, const std::array<int, D> &x, const int i) {
    ...
    //
    auto neigh  = is_curr_dir_bndry == 0 ? arg.in.template operator()<shifts[base_dir], shifts[dir] > (i) : arg.in.template get_bndry_term<dir>(x,i);
    //
    if        constexpr ( dir % 2 == 0) {
  ...
  inline Tp add_neighbors(int &face_type, const std::array<int, D> &x, const int i) {
    //
    auto current_face_type = arg.in.template check_face_type<dir>(x);
    //
    auto neigh  = current_face_type == 0 ? arg.in.template operator()<shifts[dir]> (i) : arg.in.template get_bndry_term<dir>(x,i);
